...
bool Condomino::validaHabitacao(Habitacao *h1) const {
    if (this->habitacoes.size() == 0) return false;
    vector<Habitacao *>::const_iterator it = this->habitacoes.begin();
    while (it != this->habitacoes.end()) {
...
const tm Condomino::getDataEntrada() const {
    return dataEntrada;
}

void Condomino::setDataEntrada(const tm &de) {
    dataEntrada.tm_hour = de.tm_hour;
    dataEntrada.tm_min = de.tm_min;
    dataEntrada.tm_sec = de.tm_sec;
    dataEntrada.tm_mon = de.tm_mon;
    dataEntrada.tm_mday = de.tm_mday;
    dataEntrada.tm_year = de.tm_year;
}
